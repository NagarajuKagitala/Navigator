...
public class IIBMessageFlowViewlet 
{
	...
	@Test(priority=1)
	@TestRail(testCaseId=1067)
    @Parameters({"ViewletName", "ViewletValue"})
	public static void AddIIBMessageFlowViewlet(String ViewletName, int ViewletValue, ITestContext context) throws InterruptedException
	{
		...
		{
			System.out.println("IIB Message flow Viewlet is created");
			context.setAttribute("Status",1);
			context.setAttribute("Comment", "IIB Message flow viewlet is created successfully");
		}
		else
		{
			System.out.println("IIB Message flow viewlet is not created");
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "Failed to create IIB Message flow viewlet");
	...
	@TestRail(testCaseId=1071)
	@Test(priority=17)
	public static void ShowObjectAttributesForIIBMessageFlow(String Dashboardname, String schemaName, ITestContext context) throws InterruptedException
	{
		try {
			IIBObjectAttributes obj=new IIBObjectAttributes();
		obj.IIBBrokerShowObjectAttributes(Dashboardname, driver, schemaName);
		context.setAttribute("Status",1);
		context.setAttribute("Comment", "Show object attributes for IIB Message flow viewlet is working fine");
		}
		catch(Exception e)
		{
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "Failed to Show object attributes for IIB Message flow viewlet, check details: "+ e.getMessage());
	...
	{    	
    	//Store the Message flow name into string
		String MessageFlowName=driver.findElement(By.xpath("//div[1]/app-viewlet/div/ngx-datatable/div/datatable-body/datatable-selection/datatable-scroller/datatable-row-wrapper/datatable-body-row/div[2]/datatable-body-cell[4]/div/span")).getText();
		...
		if(Favdata.contains(MessageFlowName))
		{
			System.out.println("Message flow name is added to the Favorite viewlet");
			context.setAttribute("Status",1);
			context.setAttribute("Comment", "Message flow names are  added successfully to favorite viewlet");
		}
		else
		{
			System.out.println("Message flow name is not added to the Favorite viewlet");
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "Failed to add Message flow name to favorite viewlet");
	...
	@Test(priority=11)
	@TestRail(testCaseId=1069)
	public static void CompareMessageFlows(ITestContext context) throws InterruptedException
	{
	...
	@TestRail(testCaseId = 1070)
	@Test(priority=12)
	public void CheckDifferencesForMessageFlows(ITestContext context) throws InterruptedException
	{
