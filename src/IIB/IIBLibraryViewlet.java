...
public class IIBLibraryViewlet 
{
	...
	public void beforeTest() throws Exception 
	{
		...
		String WGSName = Settings.getWGSNAME();
		String Node_Hostname = Settings.getNode_Hostname();
		IIB_WGSNAME =Settings.getIIB_WGSNAME();
		IIBNode =Settings.getIIBNode();
		String Low = Settings.getLow();
		Medium =Settings.getMedium();
		High =Settings.getHigh();
		Object LowSleep = Integer.parseInt(Low);
		int MediumSleep = Integer.parseInt(Medium);
	...
	public static void AddIIBLibraryViewlet(String ViewletName, int ViewletValue, ITestContext context) throws InterruptedException
	{
		...
		{
			System.out.println("IIB Library Viewlet is created");
			context.setAttribute("Status",1);
			context.setAttribute("Comment", "IIB Library viewlet is created successfully");
		}
		else
		{
			System.out.println("IIB Applications viewlet is not created");
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "Failed to create IIB Library viewlet");
	...
	@Test(priority=17)
	public static void ShowObjectAttributesForIIBLibrary(String Dashboardname, String schemaName, ITestContext context) throws InterruptedException
	{
		try {
			IIBObjectAttributes obj=new IIBObjectAttributes();
		obj.IIBBrokerShowObjectAttributes(Dashboardname, driver, schemaName);
		context.setAttribute("Status",1);
		context.setAttribute("Comment", "Show object attributes for IIB Library viewlet is working fine");
		}
		catch(Exception e)
		{
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "Failed to Show object attributes for IIB Library viewlet, check details: "+ e.getMessage());
