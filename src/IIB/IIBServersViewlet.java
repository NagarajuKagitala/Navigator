...
public class IIBServersViewlet 
{
	...
	@Test(priority=1)
	@TestRail(testCaseId=1059)
    @Parameters({"ViewletName", "ViewletValue"})
	public static void AddIIBServerViewlet(String ViewletName, int ViewletValue, ITestContext context) throws InterruptedException
	{
		...
		{
			System.out.println("IIB Server Viewlet is created");
			context.setAttribute("Status",1);
			context.setAttribute("Comment", "IIB Server viewlet is created successfully");
		}
		else
		{
			System.out.println("IIB Server viewlet is not created");
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "Failed to create IIB Server viewlet");
	...
	@TestRail(testCaseId=1061)
	@Test(priority=17)
	public static void ShowObjectAttributesForIIBServer(String Dashboardname, String schemaName, ITestContext context) throws InterruptedException
	{
		try {
			IIBObjectAttributes obj=new IIBObjectAttributes();
		obj.IIBBrokerShowObjectAttributes(Dashboardname, driver, schemaName);
		context.setAttribute("Status",1);
		context.setAttribute("Comment", "Show object attributes for IIB Server viewlet is working fine");
		}
		catch(Exception e)
		{
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "Failed to Show object attributes for IIB server viewlet, check details: "+ e.getMessage());
	...
	{    	
    	//Store the server name into string
		String ServerName=driver.findElement(By.xpath("//div[1]/app-viewlet/div/ngx-datatable/div/datatable-body/datatable-selection/datatable-scroller/datatable-row-wrapper/datatable-body-row/div[2]/datatable-body-cell[4]/div/span")).getText();
		...
		if(Favdata.contains(ServerName))
		{
			System.out.println("Server name is added to the Favorite viewlet");
			context.setAttribute("Status",1);
			context.setAttribute("Comment", "Server names are  added successfully to favorite viewlet");
		}
		else
		{
			System.out.println("Server name is not added to the Favorite viewlet");
			context.setAttribute("Status",5);
			context.setAttribute("Comment", "Failed to add Server name to favorite viewlet");
